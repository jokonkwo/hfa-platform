...
| Map geometry | **Direct GeoJSON for v1 (Fresno County only); PMTiles for expansion** | v1 uses Census TIGER ZCTA polygons fetched at build time and stored in `apps/api/data/fresno_zip_boundaries.geojson` (~275KB, 18 ZIPs). Served via `GET /v1/zips/boundaries`. Direct GeoJSON is appropriate at Fresno-county scale. **PMTiles (Tippecanoe → Cloudflare R2 → MapLibre) remains the plan if coverage expands beyond one county** — do not switch to PMTiles for single-county scope, the overhead is not justified. |
| Basemap | **CARTO Positron** (`https://basemaps.cartocdn.com/gl/positron-gl-style/style.json`) | Free vector style, no API key. Replaces the original OSM raster tiles. Muted, minimal aesthetic closer to the Reventure.app reference. |
| Web | Next.js + MapLibre GL JS | `apps/web` exists and is running. Map has CARTO Positron basemap, GeoJSON fill layer for ZIP boundaries colored by AQI, circle markers, sidebar, filter, detail panel, About panel. |
